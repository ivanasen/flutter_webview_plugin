...
@interface FlutterWebviewPlugin () <WKNavigationDelegate, UIScrollViewDelegate,
  ...
  UITapGestureRecognizer *_webviewDisabledRecognizer;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  channel = [FlutterMethodChannel methodChannelWithName:CHANNEL_NAME
                                        binaryMessenger:[registrar messenger]];

  UIViewController *viewController =
      [UIApplication sharedApplication].delegate.window.rootViewController;
  FlutterWebviewPlugin *instance =
      [[FlutterWebviewPlugin alloc] initWithViewController:viewController];
...
- (instancetype)initWithViewController:(UIViewController *)viewController {
  ...
  if (self) {
    ...
    _webviewDisabledRecognizer = [[UITapGestureRecognizer alloc]
        initWithTarget:self
                action:@selector(handleDisabledWebviewTapGesture:)];
  }
  return self;
}
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"launch" isEqualToString:call.method]) {
    ...
    [self reload];
    result(nil);
  } else if ([@"setWebviewTouchesEnabled" isEqualToString:call.method]) {
    [self handleWebviewTouchesEnabledCall:call];
    result(nil);
  } else {
...
- (void)initWebview:(FlutterMethodCall*)call {
  ...
  if (userAgent != (id)[NSNull null]) {
    [[NSUserDefaults standardUserDefaults]
        registerDefaults:@{@"UserAgent" : userAgent}];
  }
  ...
  UIViewController *currentViewController = presentedViewController != nil
...
- (void)evalJavascript:(FlutterMethodCall *)call
     completionHandler:
         (void (^_Nullable)(NSString *response))completionHandler {
  if (self.webview != nil) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:@"estimatedProgress"] &&
      object == self.webview) {
    [channel invokeMethod:@"onProgressChanged"
                arguments:@{@"progress" : @(self.webview.estimatedProgress)}];
  } else {
...
- (bool)checkInvalidUrl:(NSURL *)url {
  ...
  if (_invalidUrlRegex != [NSNull null] && urlString != nil) {
    ...
    NSRegularExpression *regex = [NSRegularExpression
        regularExpressionWithPattern:_invalidUrlRegex
...
- (void)handleWebviewTouchesEnabledCall:(FlutterMethodCall *)call {
  NSNumber *enabledArg = call.arguments[@"enabled"];
  if (enabledArg != (id)[NSNull null]) {
    BOOL enabled = [enabledArg boolValue];

    self.webview.scrollView.scrollEnabled = enabled;
    ...
    if (enabled == NO) {
      [self.webview.scrollView addGestureRecognizer:_webviewDisabledRecognizer];
    } else {
      [self.webview.scrollView
          removeGestureRecognizer:_webviewDisabledRecognizer];
    }
  }
}

- (void)handleDisabledWebviewTapGesture:(UITapGestureRecognizer *)sender {
  [channel invokeMethod:@"onDisabledWebviewTouched" arguments:nil];
}
...
- (void)webView:(WKWebView *)webView
  ...
  if (_enableAppScheme || ([webView.URL.scheme isEqualToString:@"http"] ||
