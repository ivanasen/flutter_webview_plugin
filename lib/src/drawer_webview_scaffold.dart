...
class DrawerWebviewScaffold extends StatefulWidget {
  const DrawerWebviewScaffold({
    Key key,
    this.appBar,
    @required this.url,
    this.headers,
  ...
  @override
  _DrawerWebviewScaffoldState createState() => _DrawerWebviewScaffoldState();
}

class _DrawerWebviewScaffoldState extends State<DrawerWebviewScaffold> {
    ...
    if (widget.hidden) {
      _onStateChanged =
        ...
        if (state.type == WebViewState.finishLoad) {
          ...
          webviewReference.takeScreenshot();
        }
      });
    }

    webviewReference.onScrollStopped.listen((Point scrollPosition) {
      webviewReference.takeScreenshot();
    });

    webviewReference.onScreenshotTaken.listen((Uint8List screenshot) {
      setState(() {
        _screenshot = Image.memory(screenshot);
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: _drawerOpened ? _screenshot : _buildWebView());
  }
